...
RUN_ROOT="${RUN_ROOT:-$PROJECT_ROOT/runs/deepsignal_cycleplan/chengdu_unbalanced_x1p2_readme_models_20260701_$(date +%Y%m%dT%H%M%S)}"
LOG_DIR="$RUN_ROOT/logs"
TLS_DIR="$RUN_ROOT/tls"
ORCH_LOG="$LOG_DIR/orchestrator.log"

TARGET_TLS="${TARGET_TLS:-J54 314655170 432452987}"
CASE_KEYS="${CASE_KEYS:-fp16 qwen9b gemma12 qwen4b}"
...
ensure_path() {
  ...
  local kind="${3:-dir}"
  if [[ "$kind" == "file" ]]; then
    [[ -f "$path" ]] || { skip_or_fail_missing "$case_key" "$path"; return 1; }
  else
...
require_gpu() {
  ...
  if ! command -v nvidia-smi >/dev/null 2>&1 || ! nvidia-smi -L >/dev/null 2>&1; then
    log_event "ERROR no_cuda_gpu case_keys='$CASE_KEYS'"
    exit 64
  fi
}
...
payload = {
    "purpose": "20260701 supplement rerun: DeepSignal-20260519 F16, Qwen3.5-9B, Gemma3-12B-IT, and Qwen3-4B on Chengdu unbalanced x1.2 T=0.2 three-TL protocol.",
    ...
    "recording_policy": "Keep all raw and aggregate metrics. README display may omit columns, but experiment artifacts retain throughput, Network ATT, and all per-step fields.",
    "runner_window": {
...
run_hf_case() {
  ...
  local case_name="$2"
  local model_path="$3"
  ensure_path "$case_key" "$model_path" dir || return 0
...
run_fp16_case() {
  ensure_path fp16 "$FP16_GGUF_PATH" file || return 0
  ensure_path fp16 "$LLAMA_SERVER" file || return 0
  run_case fp16 "03_model_fp16_20260519_unbalanced_temp02_x1p2" \
    --input-mode github_official \
    --prompt-format deepsignal \
    --deepsignal-reasoning-max-chars "$DEEPSIGNAL_REASONING_MAX_CHARS" \
    --no-prefill \
...
for case_key in $CASE_KEYS; do
  case "$case_key" in
    fp16)
      ...
      ;;
    qwen9b)
      run_hf_case "$case_key" "02_qwen35_9b_base_nochat_strict_deepsignal_unbalanced_temp02_x1p2" "$QWEN9B_PATH"
      ;;
    gemma12)
      run_hf_case "$case_key" "05_gemma3_12b_it_nochat_strict_deepsignal_unbalanced_temp02_x1p2" "$GEMMA12_PATH"
      ;;
    qwen4b)
      run_hf_case "$case_key" "04_qwen3_4b_base_nochat_strict_deepsignal_unbalanced_temp02_x1p2" "$QWEN4B_PATH"
      ;;
    *)
