...
RUN_ROOT="${RUN_ROOT:-$PROJECT_ROOT/runs/deepsignal_cycleplan/j54_qwen3_4b_base_json_repaired_smoke_20260622}"
...
{
  "run_root": "$RUN_ROOT",
  "purpose": "J54 Qwen3-4B base smoke test with JSON-only prompt and repaired online control",
  "tls": ["J54"],
...
log_event "RUN_START run_root=$RUN_ROOT model=$MODEL_PATH prompt_format=deepsignal_json online_control_mode=repaired fail_policy=keep_default"
status=0
for temp in 0.0 0.1; do
  label="$(temp_label "$temp")"
  for scale in 1.0 1.5; do
    tag="$(scale_tag "$scale")"
    run_case "04_qwen3_4b_base_json_repaired_${label}_x${tag}" "$scale" "$temp" || status=$?
  done
done
