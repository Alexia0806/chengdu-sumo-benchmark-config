...
RUN_ROOT="${RUN_ROOT:-$PROJECT_ROOT/runs/deepsignal_cycleplan/j54_qwen3_4b_base_json_chattemplate_clean_strict_smoke_20260622}"
...
MODEL_PATH="${MODEL_PATH:-$MODELS_ROOT/Qwen3-4B}"
...
{
  "run_root": "$RUN_ROOT",
  "purpose": "J54 Qwen3-4B base clean strict smoke: JSON-only prompt, HF chat template, skip special tokens, no first-JSON truncation, strict online control",
  "tls": ["J54"],
...
log_event "RUN_START run_root=$RUN_ROOT model=$MODEL_PATH prompt_format=deepsignal_json online_control_mode=strict json_stop_after_first=false hf_use_chat_template=true hf_skip_special_tokens=true fail_policy=keep_default"
status=0
for temp in 0.0 0.1; do
  label="$(temp_label "$temp")"
  for scale in 1.0 1.5; do
    tag="$(scale_tag "$scale")"
    run_case "04_qwen3_4b_base_json_chattemplate_clean_strict_${label}_x${tag}" "$scale" "$temp" || status=$?
  done
done
