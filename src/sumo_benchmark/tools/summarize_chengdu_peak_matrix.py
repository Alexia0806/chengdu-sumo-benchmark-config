...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize Chengdu benchmark matrix outputs.")
    parser.add_argument("run_root", type=Path, help="Benchmark run root containing case subdirectories.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for per_tl in sorted(root.glob("*/per_tl.jsonl")):
        ...
        for row in rows:
            tl_row: dict[str, Any] = {
                "case": case_dir.name,
                "group": group,
                "demand_scale": scale or row.get("demand_scale"),
                "tl_id": row.get("tl_id"),
                "failures": len(load_jsonl(case_dir / "failures.jsonl")),
            }
            for key in METRICS:
                tl_row[key] = row.get(key)
            tl_row["parse_error_total"] = sum(int(v) for v in (row.get("parse_errors") or {}).values())
            tl_row["relaxed_parse_error_total"] = sum(
                int(v) for v in (row.get("relaxed_parse_errors") or {}).values()
            )
            tl_row["repair_error_total"] = sum(int(v) for v in (row.get("repair_errors") or {}).values())
        ...
        parse_total = 0
        relaxed_parse_total = 0
        repair_error_total = 0
        for row in rows:
            parse_total += sum(int(v) for v in (row.get("parse_errors") or {}).values())
            relaxed_parse_total += sum(int(v) for v in (row.get("relaxed_parse_errors") or {}).values())
            repair_error_total += sum(int(v) for v in (row.get("repair_errors") or {}).values())
        summary["parse_error_total"] = parse_total
        summary["relaxed_parse_error_total"] = relaxed_parse_total
        summary["repair_error_total"] = repair_error_total
    ...
    fieldnames.extend(["parse_error_total", "relaxed_parse_error_total", "repair_error_total"])
    with csv_path.open("w", encoding="utf-8", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(summary_rows)

    per_tl_csv_path = root / "matrix_per_tl_summary.csv"
    per_tl_fieldnames = ["case", "group", "demand_scale", "tl_id", "failures"]
    per_tl_fieldnames.extend(METRICS)
    per_tl_fieldnames.extend(["parse_error_total", "relaxed_parse_error_total", "repair_error_total"])
    with per_tl_csv_path.open("w", encoding="utf-8", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=per_tl_fieldnames)
        writer.writeheader()
    ...
    for row in sorted(summary_rows, key=lambda item: (float(item["demand_scale"]), item["group"])):
        ...
        print(
            "| {group} | {scale} | {tls} | {applied_rate} | {fallback_rate} | {strict_control} | {relaxed_control} | {repaired_control} | {response_time} | {queue} | {delay} | {throughput} | {target_att} | {target_awt} | {network_att} | {network_awt} | {over} | {fallback} | {parse} |".format(
                ...
                applied_rate=fmt(row.get("plans_applied_rate_mean")),
                fallback_rate=fmt(row.get("fallback_plan_rate_mean")),
                strict_control=fmt(
                    first_non_none(
                        row.get("strict_control_usable_rate_mean"),
                        row.get("control_usable_rate_mean"),
                    )
                ),
    ...
    for row in sorted(tl_rows, key=lambda item: (float(item["demand_scale"]), item["group"], str(item.get("tl_id")))):
        ...
        def first_non_none_tl(*values: Any) -> Any:
            for value in values:
                if value is not None:
                    return value
            return None
        ...
        print(
            "| {group} | {scale} | {tl_id} | {applied_rate} | {fallback_rate} | {strict_control} | {response_time} | {queue} | {delay} | {throughput} | {target_att} | {target_awt} | {network_att} | {network_awt} | {fallback} | {parse} |".format(
                ...
                applied_rate=fmt_tl(row.get("plans_applied_rate")),
                fallback_rate=fmt_tl(row.get("fallback_plan_rate")),
                strict_control=fmt_tl(
                    first_non_none_tl(
                        row.get("strict_control_usable_rate"),
                ...
                target_awt=fmt_tl(row.get("target_tl_awt_sec")),
                network_att=fmt_tl(row.get("network_att_sec")),
                network_awt=fmt_tl(row.get("network_awt_sec")),
                fallback=fmt_tl(row.get("fallback_plans_applied")),
                parse=row["parse_error_total"],
            )
        )
    return 0
